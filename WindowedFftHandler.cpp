...
WindowedFftHandler :: WindowedFftHandler(long N_In, long M_In, SAMPLE* windowIn) : FftBase(N_In, M_In)
{
	...
	strcpy(windowName, "User Defined");
...
void WindowedFftHandler :: findUnitaryFftMagnitudes(SAMPLE* inBuffer, SAMPLE* outBuffer)
{
  ...
  //Compute fft since we now have our real input stored as the .real values in bit-reversed order and all the .imag values set to 0.0.
	...
	for (int i = 0; i < M; i++)
	{
		...
		outBuffer[i] = (SAMPLE)(complexMag(tempBuffer[i])); //spi
	}
}
...
{	
  ...
  //Compute fft since we now have our real input stored as the .real values in bit-reversed order and all the .imag values set to 0.0.
	...
	for (int i = 0; i < M; i++)
	{
		...
		outBuffer[i] = (SAMPLE) (20.0 * log10(complexMag(tempBuffer[i])) + normalizationFactor); //spi
	}
}
...
void WindowedFftHandler :: setWindowToHann()
{
	...
	for( int n = 0; n < N; n++)
		window[n] = (SAMPLE) (normalizationFactor * ( 0.5 * (1.0 - cos(omega * n)) )); //spi
}
...
void WindowedFftHandler :: setWindowToHamming()
{
	...
	for( int n = 0; n < N; n++)
		window[n] = (SAMPLE) (normalizationFactor * ( 0.54 - 0.46 * cos(omega * n))); //spi
}
...
void WindowedFftHandler :: setWindowToBlackman()
{
	...
	for( int n = 0; n < N; n++)
		window[n] = (SAMPLE) (normalizationFactor * ( 0.42 - 0.5 * cos(omega * n) + 0.08 * cos(2 * omega * n))); //spi
}