...
{
	delete[] realValuedInputBuffer; //spi, test
	delete[] magnitudeValues; //spi, test
	delete[] xAxisValues; //spi, test
	delete[] axisType; //spi, test
}
...
void SpectrumDisplay :: makeXAxisDisplayLog()
{
		...
		for (int binNum = 1; binNum < M/2; binNum++)
		{
			...
			xAxisValues[binNum] = (SAMPLE) pow(binNum * (SRATE / (2.0 * getM())), 0.3); //spi
		}
}
...
void SpectrumDisplay :: makeXAxisDisplayLinear()
{
	for (int binNum = 0; binNum < M/2; binNum++)
	{
		...
		xAxisValues[binNum] = (SAMPLE) binNum * ((float)SRATE / getM()); //spi
	}
}